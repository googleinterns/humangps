...
from google3.vr.perception.deepholodeck.human_correspondence.data.smpl import utils as smpl_utils
...
class ProtoPathToTFExampleFn(beam.DoFn):
  ...
  def __init__(self, image_height: int, image_width: int,
    ...
    self._smpl_pair_generator = smpl_utils.SMPLPairGenerator()
  ...
  def process(
    ...
    (mesh1, mesh2) = self._smpl_pair_generator.generate_smpl_pair(sample_pair)
    ...
    seq_name1 = sample_pair[0]['pose_info']['seq_name']
    seq_name2 = sample_pair[1]['pose_info']['seq_name']
    frame_idx1 = str(sample_pair[0]['pose_info']['frame_idx'])
    frame_idx2 = str(sample_pair[1]['pose_info']['frame_idx'])

    try:
      feature = {}
      # Add RGB, Image are in [0, 255].
      human_data_utils.add_rgb_rendering(feature, mesh1['mesh'],
                                         mesh1['texture_map'], source_camera,
                                         'source')
      human_data_utils.add_rgb_rendering(feature, mesh2['mesh'],
                                         mesh2['texture_map'], target_camera,
                                         'target')
      # Add mask
